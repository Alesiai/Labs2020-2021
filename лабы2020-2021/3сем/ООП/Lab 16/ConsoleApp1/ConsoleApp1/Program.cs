    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Console.WriteLine($"\nTask {i}. ID: {task.Id.ToString()}");
                Console.WriteLine($"\nTask  {i}. статус: {task.Status.ToString()}");
                task.Wait();
                stopwatch.Stop();
                Console.WriteLine($"\nTime of ending task {i}: {stopwatch.Elapsed.TotalMilliseconds.ToString()}\n");
                stopwatch.Reset();
            }
            ...
            Console.WriteLine("q - for end ");
            if (Console.ReadKey().KeyChar == 'q')
                source.Cancel();
            task.Wait();
            stopwatch.Stop();
            Console.WriteLine($"\nTime to ending task: {stopwatch.Elapsed.TotalMilliseconds.ToString()}\n");
            ...
            stopwatch.Stop();
            Console.WriteLine($"\nEding time: {stopwatch.Elapsed.TotalMilliseconds.ToString()}");
            stopwatch.Reset();
            ...
            stopwatch.Stop();
            Console.WriteLine($"Eding time: {stopwatch.Elapsed.TotalMilliseconds.ToString()}\n");
            stopwatch.Reset();
